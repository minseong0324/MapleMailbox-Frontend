...
const StampList: React.FC<NowDateProps> = ({ nowDate }) => {
  ...
  const fetchStampStatus = useCallback(async () => {
    ...
    try {
      const response = await axios.get(`http://maplemailbox.com/api/users/${userId}/missions`, {
        headers: {
  ...
  const handleOpenMissionModal = async () => {
    if(nowDate !== null) {
      if(stampsStatus[nowDate-1] === false) {
        ...
        try {
          const response = await axios.get(`http://maplemailbox.com/api/users/${userId}/missions/todayMission`, {
            headers: {
              'authorization': `${accessToken}`
            }
          });
  ...
  const handleMissionComplete = async () => {
    ...
    try {
      ...
      const putResponse = await axios.put(`http://maplemailbox.com/api/users/${userId}/missions/todayMission`,{
      ...
      if (putResponse.status === 200) {
        try {
          ...
          const getResponse = await axios.get(`http://maplemailbox.com/api/users/${userId}/missions`, {
            headers: {
              'authorization': `${accessToken}`
            }
          });
